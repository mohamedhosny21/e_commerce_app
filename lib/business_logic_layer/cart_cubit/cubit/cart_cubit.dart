...
class CartCubit extends Cubit<CartState> {
  ...
  Future<void> insertIntoCartDatabase(PizzaModel pizzaModel) async {
    final savedUserId = await SharedPreferencesHelpers.getString('user_id');
    database
  ...
  Future<void> deleteAllCartItems() async {
    final savedUserId = await SharedPreferencesHelpers.getString('user_id');
    database.rawDelete('DELETE FROM Cart WHERE UserID=?', [savedUserId]).then(
        (value) {
      debugPrint('All items are deleted successfully from cart database');
      getFromCartDatabase(database);
    }).catchError((error) {
      debugPrint(error.toString());
    });
